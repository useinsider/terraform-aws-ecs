...
module "container_definition" {
  ...
  enable_execute_command  = coalesce(each.value.enable_execute_command, var.container_definition_defaults.enable_execute_command, var.enable_execute_command)
  ...
  memoryReservation      = try(coalesce(each.value.memory_reservation, var.container_definition_defaults.memoryReservation), null)
  mountPoints            = try(coalesce(each.value.mount_points, var.container_definition_defaults.mountPoints), null)
  name                   = coalesce(each.value.name, each.key)
  portMappings           = try(coalesce(each.value.port_mappings, var.container_definition_defaults.portMappings), null)
